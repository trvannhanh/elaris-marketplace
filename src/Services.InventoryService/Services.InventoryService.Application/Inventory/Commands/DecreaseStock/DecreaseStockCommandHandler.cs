...
{
    public class DecreaseStockCommandHandler : IRequestHandler<DecreaseStockCommand, InventoryItemDto>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (item.AvailableQuantity < request.Quantity)
            {
                throw new InvalidOperationException(
                    $"Insufficient available stock. Available: {item.AvailableQuantity}, Requested: {request.Quantity}");
            }
            ...
            _logger.LogInformation(
                "[DecreaseStock] User {UserId} decreased stock for {ProductId} by {Quantity}. New quantity: {NewQuantity}",
